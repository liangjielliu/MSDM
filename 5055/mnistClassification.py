...
class Linear(Node):
    ...
    def __init__(self, input_shape, output_shape, weight=None, bias=None):
        ...
        if weight is None:
            ...
            weight = (np.random.rand(input_shape, output_shape) * 2 - 1) * limit
...
class Softmax(Node):
    ...
    def backward(self, delta):
        '''
        Vectorized implementation to compute the gradient of loss w.r.t input x.
        For each sample, the gradient is:
          dx = s * (delta - sum(delta * s))
        where s is the softmax output.
        '''
        s = self.output  # shape: (batch_size, num_classes)
        dx = s * (delta - np.sum(delta * s, axis=1, keepdims=True))
        return dx


class CrossEntropy(Node):
    '''
    CrossEntropy loss function (one-hot encoded labels)
    '''
    def __init__(self):
    ...
    def forward(self, x, y):
        '''
        x: predicted probabilities, shape (batch_size, num_classes)
        y: one-hot encoded labels, shape (batch_size, num_classes)
        '''
        # 对预测概率做数值稳定性处理
        self.y_pred = np.clip(x, 1e-12, 1.0)
        self.y_true = y

        # 计算每个样本的负对数似然损失
        sample_loss = -np.log(self.y_pred) * self.y_true
        # 按行求和（因为one-hot编码只有正确类别有贡献），再求均值
        loss = np.mean(np.sum(sample_loss, axis=1))
        return loss

    def backward(self, delta=1):
        # 交叉熵损失对 softmax 输出的梯度为： (y_pred - y_true) / batch_size
...
if __name__ == '__main__':
    ...
    net = [Linear(dim_img, hidden_units), Sigmoid(), Linear(hidden_units, num_digit), Softmax(), CrossEntropy(), Mean()]
    ...
    for epoch in range(num_epoch):
        for j in range(num_iteration):
            ...
            for term in net:
                for i, para in enumerate(term.parameters):
                    para -= learning_rate * term.parameters_deltas[i]
